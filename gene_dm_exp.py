...
dmdata = pd.read_excel('dmdata.xlsx')
...
vae_gene.load_state_dict(torch.load('vae_gene_GD.pth'))
vae_methylation.load_state_dict(torch.load('vae_dm_GD.pth'))
...
print("\nEXPERIMENT : GENE + PREDICTED DNA METHYLATION : \n")
...
print("\nEXPERIMENT : DNA METHYLATION + PREDICTED GENE : \n")
...
hidden_dims = [ 512, 256, 128, 64]
classifier = Classifier(input_dim=128, hidden_dims=hidden_dims)
train_and_evaluate(predicted_gene_data, X_dm, labels_dna, fusion_model, classifier,num_epochs=80,learning_rate=1e-4)
...
print("\nEXPERIMENT : DNA METHYLATION : \n")

hidden_dims = [512, 256, 128, 64]
classifier = Classifier(input_dim=X_dm.shape[1], hidden_dims=hidden_dims)
train_and_evaluate_single_omics(X_dm, labels_dna, classifier)
